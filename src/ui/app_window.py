...
class App(ctk.CTk):
    ...
    def start_conversion(self):
        ...
        if ext == "txt" and target == "Binário":
            ...
            if target == "PDF": sucesso, resultado = FileConverter.image_to_pdf(self.current_file_path)
            elif target == "OCR na Nuvem (API)": sucesso, resultado = FileConverter.ocr_via_api(self.current_file_path)

        elif ext == "pdf":
            if target in ["PNG", "JPEG"]: sucesso, resultado = FileConverter.pdf_to_images(self.current_file_path, target.lower())
            elif target == "SVG": sucesso, resultado = FileConverter.pdf_to_svg(self.current_file_path)
            elif target == "Extrair Imagens": sucesso, resultado = FileConverter.extract_images_from_pdf(self.current_file_path)
            elif target == "Markdown (.md)": sucesso, resultado = FileConverter.pdf_to_markdown(self.current_file_path)
            elif target == "OCR na Nuvem (API)": sucesso, resultado = FileConverter.ocr_via_api(self.current_file_path)

        # Resultado
        if sucesso: self.file_label.configure(text=f"✅ {resultado}", text_color="green")
        else: self.file_label.configure(text=f"❌ {resultado}", text_color="red")